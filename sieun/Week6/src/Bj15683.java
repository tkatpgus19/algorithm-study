...
public class Bj15683 {
	...
	public static void main(String[] args) throws IOException {
		...
		N = Integer.parseInt(st.nextToken()); //세로입력  
		M = Integer.parseInt(st.nextToken()); //가로 입력  
		
		map = new int[N][M];  // map 생성  
		cctvList = new ArrayList<>(); //CCTV 정보들을 모아둔 list 생
		...
		for(int i = 0; i < N; i++) {
			...
			for(int j = 0; j < M; j++) {
				...
				//cctv 발견하면 cctv list에 추가  
				if(map[i][j] != 0 &&  map[i][j] != 6) {
					cctvList.add(new CCTV(map[i][j], i, j)); //cctv 종류, 위치 저
				}
			}
		}
	...
	public static void watch(CCTV cctv, int d) {
		...
		queue.add(cctv); //초기 cctv 
		visited[cctv.x][cctv.y] = true; //방문 처리 

		while(!queue.isEmpty()) {
			int nx = queue.peek().x + dx[d]; //x값 가져오기  
			int ny = queue.poll().y + dy[d]; //y값 가져오기 
			...
			if(copyMap[nx][ny] == 0) {  
				...
				queue.add(new CCTV(cctv.num, nx, ny)); //queue에 추가 해서 또 탐색하게 한다. 
			} 
	...
	public static void getBlindSpot() {
		...
		answer = Math.min(answer, cnt);  //작은값 반환하기  
	}
}
